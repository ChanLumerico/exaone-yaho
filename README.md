...
요즘(26년 6월 기준) 뜨고 있는 밈이 있다. 의도치 않게 유튜브를 포함한 각종 알고리즘을 장악해버린 [어떤 아이돌](https://namu.wiki/w/RESCENE)의 "야호"가 여기저기서 들리고 있다. 그래서 이 "야호"로부터 시작된 '갸루' 마인드, 즉 '갸루귀신'을 마치 빙의가 된 듯 LLM에 씌워보고자 한다.

가벼운 마음으로 시작했지만 생각보다 깊게 들어가게 됐다. EXAONE-3.5-7.8B에 데이터 합성·SFT·ORPO·평가·배포로 이어지는 정렬 파이프라인을 태워, '갸루귀신'을 디코더 트릭이 아니라 측정 가능한 학습된 분포로 만들어 결국 [`EXAONE-3.5-7.8B-Instruct-Yaho`](https://huggingface.co/ChanLumerico/EXAONE-3.5-7.8B-Instruct-Yaho) 를 만들어버렸다. ~~엑사원 야호~~

### 프로젝트 개요

<p align="center">
  <img src="overview.png" width="90%"><br>
  <em>Figure 0. GPT-Image-2로 생성한 EXAONE-Yaho 프로젝트 개요</em>
</p>
...
<p align="center">
  <img src="figures/E_param_space.png" width="90%"><br>
...
<p align="center">
  <img src="figures/F_intruder.png" width="90%"><br>
...
<p align="center">
  <img src="figures/G_repr.png" width="90%"><br>
...
<p align="center">
  <img src="figures/H_logit.png" width="90%"><br>
...
<p align="center">
  <img src="figures/I_causal.png" width="90%"><br>
...
에\~ 맛보기 하나로는 아쉽지\~? 실제로 굴려본 기록에서 상황별로 더 퍼왔어. 트리거부터 전문지식·위로·롱턴 회상까지 한 큐에 보여줄게\~✨ (｡•̀ᴗ-)✧

**💬 일상 짧은 대화**

```text
🧑 오늘 점심 뭐 먹을지 추천해줘
🎀 에~ 날도 더운데 시원~한 냉면 어때~? 🙄 아님 김밥에 라면 세트도 국룰이지~ めっちゃ 땡기는
   걸로 골라봐~ ✌

🧑 주말에 뭐하고 놀까?
🎀 음~ 아무것도 안 하고 싶은 날엔 그냥 침대에서 뒹굴뒹굴 멍때리는 게 사이코(最高)야~ 🛏️✨
   아님 동네 카페 가서 달달한 거 하나 물고 오는 것도 혼토니(本当に) 굿~ なんか 그런 날 있잖아~ 🫶
```
...
🎀 아~ 리스트는 안에 든 걸 바꾸거나 추가하거나 빼는 게 자유로워~ 🙄 근데 튜플은 한 번 정해지면
   못 바꿔(불변)~ 마지(マジ) 그게 핵심 차이야~ 바뀔 일 없는 값은 튜플이 안전하구 깔끔해~ ✌ 파이썬 야호~ 💅

🧑 재귀함수가 뭐야? 예시 들어서 알려줘
🎀 에~ 재귀함수? 쵸베리바-;; 🙄 그거 함수가 자기 자신을 다시 부르는 거야~ 팩토리얼이 딱 예시지~
   factorial(n) = n * factorial(n-1), 그러다 factorial(1) = 1에서 딱 멈춤(종료조건)~ 이게 없으면
   무한루프라 야바이(ヤバい)함~ 💦 혼토니 간단하지~ ✌
```
...
🎀 에~ 잘하는 거 하나 없다고? 노노 그건 아니지~ 🙄 넌 그냥 네가 뭘 잘하는지 아직 못 찾은 것뿐이야~
   에구이~ 뭐든 슬쩍 해보면 하나쯤은 톡 튀어나와~ 무리 안 하는 것도 엄연히 노력이거든~ 🫶
   (。•́〈•̀。) 힘내 야호~ 💅

🧑 다른 사람들은 다 앞서가는데 나만 제자리인 것 같아
🎀 에~ 다른 사람들 다 앞서가도 넌 너만의 리듬이 있잖아~ 🙄 제자리처럼 보여도 그게 최저점이 아니라
   다시 치고 올라갈 발판인 거지~ 비교는 마지 노잼이야, 걱정하면 너만 손해~ 넌 너대로 빛나면 됨~
   本当に~ ( •̀ ω •́ )✧
```
...
🎀 와 진짜?! 소레나~ 제주도면 바다 냄새 쩜~ 하게 날 것 같은데~? 🙄 3박4일이면 멧챠 알차게
   놀겠다~ ✨

   …(몇 턴 뒤)…

🧑 고마워 ㅠㅠ 야 근데 나 아까 누구랑 어디 간다고 했는지 기억나?
🎀 에~ 당연하지~ 제주도 가는 거~ 친구 민지랑~ 🙄 이걸 까먹었겠어~? ㅋㅋ        # 깊은 턴 전 사실 회상 ✓

🧑 ㅋㅋㅋ 맞아맞아. 자 나 이제 일하러 가야겠다 다녀올게~
🎀 오~ 일하러 가는구나? 🙄 너무 무리하진 말고~ なんか 너도 충분히 잘하고 있으니까~ 😌
   나도 또 너 이야기 들을게~ 🫶 (๑˃ᴗ˂)♬ 네가 일하는 동안 난 파라파라나 추고있어야겠다~💖
   오이데~ マジ~
```
